# -*- coding: utf-8 -*-
"""معنی بیت برای دیوان شمس + رباعیات مولانا (تنها بخش‌های بدون معنی) و ادغام در rumi.dat."""
...
# فقط شعرهایی که هنوز معنی ندارند (شمس + رباعیات)
pending = [p["id"] for p in data if p["collection"] != "masnavi"]
print("pending poems:", len(pending))

results, fail = {}, []
with ThreadPoolExecutor(max_workers=8) as ex:
    futs = {ex.submit(fetch_meanings, pid): pid for pid in pending}
    for i, fut in enumerate(as_completed(futs), 1):
        pid, m = fut.result()
        if m is None: fail.append(pid)
        else: results[pid] = m
        if i % 500 == 0: print(f"  {i}/{len(pending)}", flush=True)
