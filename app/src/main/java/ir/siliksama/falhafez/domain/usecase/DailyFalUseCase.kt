 ...
 * همیشه فقط از «غزلیات» — تا با افزوده‌شدنِ بخش‌های دیگرِ دیوان، شعرِ روز ثابت بماند.
 */
class DailyFalUseCase @Inject constructor(
    ...
    suspend fun today(): Poem? {
        ...
        return poemRepository.getPoemAtForCollection(Poet.HAFEZ, Collection.HAFEZ_GHAZAL, index)
    }
    ...
    suspend fun poemForDay(dayNumber: Long): Poem? {
        ...
        return poemRepository.getPoemAtForCollection(
            Poet.HAFEZ, Collection.HAFEZ_GHAZAL, (dayNumber % count.toLong()).toInt()
        )
    }
