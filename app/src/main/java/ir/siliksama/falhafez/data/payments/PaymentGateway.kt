...
interface PaymentGateway {
    ...
    /**
     * قیمتِ **واقعیِ** محصولات، همان‌طور که در پنلِ کافه‌بازار تنظیم شده.
     *
     * چرا لازم است: قیمت‌های داخلِ [SupportTier.priceToman] هاردکد هستند. اگر
     * قیمت در پنلِ بازار عوض شود (یا با تخفیفِ فروشگاهی فرق کند)، اپ عددی را
     * نشان می‌دهد که با مبلغِ صفحهٔ پرداخت نمی‌خواند — بدترین حالتِ ممکن در یک
     * جریانِ پرداخت، چون کاربر حس می‌کند فریب خورده.
     *
     * [onResult] نگاشتِ sku → متنِ قیمتِ آمادهٔ نمایش (رشته‌ای که خودِ بازار
     * می‌دهد). نگاشتِ خالی یعنی «نتوانستیم بپرسیم» و باید به قیمتِ محلی برگردیم.
     */
    fun fetchPrices(context: Context, onResult: (Map<String, String>) -> Unit)
}
